...
- **Record it again** — expand any recently finished session and press **Record again**. PVArr loads that session's settings — teams, candidate URLs, headers, freeze timeout and the length you asked for — back into the new-recording form, writing to a **new file** so the finished recording is untouched. It fills the form rather than starting immediately, on purpose: candidate URLs usually carry a token that expires within hours, so you get to replace a stale one instead of watching the probe fail. Cookies are the exception and are never sent back to the browser — a candidate that used one is flagged so you can paste it in again. Only sessions still listed on the dashboard can be offered; PVArr keeps the last 20 and forgets them when it restarts.
...
- **Sports-friendly auto-naming** — derives readable filenames for broadcasts instead of opaque timestamps. The resolution at the end of the name (`_1080p`, `_720p`) is checked against the actual video when the recording finishes: the form's **Resolution Tag** is only a guess made before any video arrives, so a stream that turns out to be 1280x720 finishes as `..._720p.mp4` even if the form said 1080p. The `.ts` keeps the form's tag while it is recording. A file you have renamed yourself is left alone, and if the corrected name is already taken the new file gets `_1` rather than overwriting anything.
